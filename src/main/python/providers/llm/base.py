...
def get_llm_provider(model_name: str, provider: Optional[str] = None,
    ...
    if provider == LLMProvider.OPENAI:
        from src.main.python.providers.llm.openai import OpenAILLM
        return OpenAILLM(model_name=model_name, api_key=api_key,
                         base_url=base_url, temperature=temperature, **kwargs)

    if provider == LLMProvider.OLLAMA:
        from src.main.python.providers.llm.ollama import OllamaLLM
    ...
    raise ValueError(
        ...
        "如需新增提供商，请继承 providers.llm.base.BaseLLM 并在此函数中注册。"
    )
