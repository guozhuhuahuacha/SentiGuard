...
用法：
    python src/test/python/test_report_html.py
    python src/test/python/test_report_html.py --claim "你的声明"
    python src/test/python/test_report_html.py --claim "你的声明" --output my_report.html
"""
import argparse
import json
import os
...
def run(claim: str, model: str = "doubao/doubao-seed-2-0-mini-260428", output: str = ""):
    ...
    support_count = sum(1 for e in evidence_items if e.relationType == "support")
    attack_count = sum(1 for e in evidence_items if e.relationType == "attack")
    ...
    # 6. 生成 HTML 报告
    print(">>> 正在生成 LLM 叙事 HTML 报告（约 30-40s）...")
    try:
    ...
    # 7. 保存文件
    output_path = output or f"report_{agent.trace.run_id}.html"
